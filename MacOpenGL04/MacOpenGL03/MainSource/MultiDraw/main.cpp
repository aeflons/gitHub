...
void Init()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertColor) + sizeof(vertices)+ 1 * sizeof(vertPosOffsets), NULL, GL_STATIC_DRAW);
    ...
    glVertexAttribPointer(PosOffset, 3, GL_FLOAT, GL_FALSE, sizeof(vertPosOffsets[0]), BUFFER_OFFSET(sizeof(vertices)+sizeof(vertColor)));
    glEnableVertexAttribArray(PosOffset);
    glVertexAttribDivisor(PosOffset, 1);//
}
void Display()
{
